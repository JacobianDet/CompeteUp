...
int dfs_sol2(int s, int p, int ht)
{
	upa.pb(ht);
	int mht = ht;
	for(auto u : G[s])
...
ll dfs_sol1(int s, int compsz)
{
	...
	for(auto u : G[s])
	{
		if(isCrtd[u])
		continue;
		mht = max(mht, dfs_sol2(u, s, 1));
		for(auto &&e : upa)
		{
			int l = (lower_bound(begin(px), end(px), e) - begin(px));
			int r = (upper_bound(begin(px), end(px), 2*mht) - begin(px));
			if(px[l] == e)
			rz++;	
			if(l > r)
			continue;
...
ll dfs_vis1(int s)
{
	...
	ll ax = dfs_sol1(centroid.ff, sze[s]);
...
void solve(int T)
{
	...
	// cout<<ax<<" "<<nx<<"\n";
	SETF();
	cout<<setprecision(10)<<((ld)ax/(ld)nx)<<"\n";
	UNSETF();
	return;
}
