...
void solve(int T)
{
	...
	while(q--)
	{
		...
		if(x == 'P')
		{
			...
			{
				D1.update(1, 1, n, disc[head[l]], disc[l]);
				l = P[head[l]][0];
			}
			if(l != lc)
			D1.update(1, 1, n, disc[lc]+1, disc[l]);
			while(head[lc] != head[r])
			{
				D1.update(1, 1, n, disc[head[r]], disc[r]);
				r = P[head[r]][0];
			}
			if(r != lc)
			D1.update(1, 1, n, disc[lc]+1, disc[r]);	
		}
		else
		{
			ll ans = 0;
			...
			while(head[lc] != head[l])
			{
				ans += D1.query(1, 1, n, disc[head[l]], disc[l]);
				l = P[head[l]][0];
			}
			if(l != lc)
			ans += D1.query(1, 1, n, disc[lc]+1, disc[l]);
			while(head[lc] != head[r])
			{
				ans += D1.query(1, 1, n, disc[head[r]], disc[r]);
				r = P[head[r]][0];
			}
			if(r != lc)
			ans += D1.query(1, 1, n, disc[lc]+1, disc[r]);
			cout<<ans<<"\n";
		}
	}
	for(int i=1;i<=n;i++)
