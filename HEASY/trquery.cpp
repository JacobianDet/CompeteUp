...
void solve(int T)
{
    ...
    dfs_visit(1, -1);
    VI xfd(n);
    iota(begin(xfd), end(xfd), 1);
    sort(begin(xfd), end(xfd), [](int A, int B){ return (lvl[A] < lvl[B]); });
    int ct = 1;
    for(auto &&u : xfd)
    {
        cord[u] = ct;
    ...
    while(q--)
    {
        ...
        if(--t)
        {
            ...
            auto bs1 = [&](void)->int{
                ...
                {
                    int mid = lo + (hi - lo)/2;
                    if(D.query(1, 1, n, 1, mid) > p)
                    hi = mid;
                    else lo = mid + 1;
                }
                if(D.query(1, 1, n, 1, lo) <= p)
                return -1;
                else return lvl[dorc[lo]];
            };
            cout<<bs1()<<"\n";
        }
        else
