...
void solve(int T)
{
	...
	cin>>arr[i];	
	VI axf(n);
	iota(begin(axf), end(axf), 1);
	sort(begin(axf), end(axf), [](int A, int B){ return ((arr[ar[A]] > arr[ar[B]]) || ((arr[ar[A]] == arr[ar[B]]) && (ar[A] < ar[B]))); });
	ll sx = 0;
	segtree D;
	D.build(1, 1, m);
	for(auto &&u : axf)
	{
		PLL vf = D.query(1, 1, m, 1, ar[u]);
		if(vf.ff < arr[ar[u]])
		{
			sx += vf.ff;
			D.update(1, 1, m, vf.ss);
		}	
		else sx += arr[ar[u]];
	}
	cout<<sx<<"\n";
