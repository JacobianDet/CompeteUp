...
void solve(int T)
{
    ...
    cin>>r1>>r2;
    for(auto &&u : edg)
    u.ss = abs(arr[u.ff.ff] - arr[u.ff.ss]);
    sort(begin(edg), end(edg), [](PPII A, PPII B){ return (A.ss < B.ss); });
    dsu D;
    for(auto &&u : edg)
    {
        if(D.find_union(u.ff.ff, u.ff.ss))
        {
            G[u.ff.ff].pb({u.ff.ss, u.ss});
            G[u.ff.ss].pb({u.ff.ff, u.ss});
        }
    }
    dfs_visit(r1, -1);
    cout<<fz[r2]<<"\n";
