...
void dfs_sol2(int s, int p, int pf)
{
	upa.pb({pf, !!dst[pf + MV]});
	dst[pf + MV]++;
	for(auto u : G[s])
...
ll dfs_sol1(int s, int compsz)
{
	...
	for(auto u : G[s])
	{
		...
		{	
			if(e.ss)
			{
				if(e.ff == 0)
				rz++;	
				rz += (arr[0][-e.ff + MV] + arr[1][-e.ff + MV]);
			}	
			else 
			{
				if(e.ff == 0)
	...
	for(int i=-compsz+1;i<=compsz-1;i++)
	{
		arr[0][i + MV] = 0;
		arr[1][i + MV] = 0;
	}	
