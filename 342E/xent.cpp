...
int ar[MV], par[MV], sze[MV], lvl[MV], P[MV][LMV];
bool isCrtd[MV];
ll arr[MV];
VI G[MV];
...
void CDS::update(int u)
{
	int x = u;
	while(x != -1)
	{
		ar[x] = min(ar[x], lvl[x] + lvl[u] - 2*lvl[D.LCA_find(x, u)]);
		x = par[x];
	}
	return;
}
...
int CDS::query(int u)
{
	...
	while(x != -1)
	{
		rz = min(rz, ar[x] + lvl[x] + lvl[u] - 2*lvl[D.LCA_find(x, u)]);
		x = par[x];
	}
	return rz;
}
