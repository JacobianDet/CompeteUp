...
void siever(void)
{
	...
	for(ll i=2;i<MV;i++)
	{
		if(lo[i/lo[i]] == lo[i])
		{
			...
			if(lo[j/lo[j]] == lo[i])
			prx[i] = 0;
			else prx[i] = ((prx[j/lo[j]]%MOD) * ((MOD - inv[lo[i]])%MOD))%MOD;	
		}	
		else prx[i] = ((prx[i/lo[i]]%MOD) * ((MOD - inv[lo[i]] + 1)%MOD))%MOD;
		psx[i] = ((psx[i-1]%MOD) + (((((((i%MOD) * (i%MOD))%MOD) * (i%MOD))%MOD) * (prx[i]%MOD))%MOD))%MOD;
	}
	return;	
}
