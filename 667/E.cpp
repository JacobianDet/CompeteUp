...
void solve(int T)
{
	...
	for(int i=1;i<=n;i++)
	cin>>axd[i].ss;
	sort(axd+1, axd+n+1, [](PLL A, PLL B){ return (A.ff < B.ff); });
	int px = 1;
	for(int i=1;i<=n;i++)
	{
		while((px <= n) && ((axd[i].ff + k) >= axd[px].ff))
		px++;	
		ar[i] = px-1;
	}
	px = n;
	for(int i=n;i>0;i--)
	{
		while((px > 0) && ((axd[px].ff + k) >= axd[i].ff))
		px--;
		br[i] = px+1;	
	...
	for(int i=1;i<=n;i++)
	{
		int lmx = (ar[i] - i + 1), dmx = 0;
		if(ar[i] < n)
		dmx = max(dmx, D.query(1, 1, n, ar[i]+1, n));
		if(br[i] > 1)
		dmx = max(dmx, D.query(1, 1, n, 1, br[i]-1));
		// whoami(i, axd[i].ff, axd[i].ss, lmx, dmx, ar[i], br[i]);
		gmx = max(gmx, lmx + dmx);	
	}	
	cout<<gmx<<"\n";
	return;
